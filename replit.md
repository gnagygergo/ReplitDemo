...
- **UI Components**: 
  ...
  - `NumberField` - Self-contained numeric input component with internal FormLabel and FormControl rendering. Supports edit/view/table modes, automatic formatting with thousands separators, percentage display (appends "%" in view/table mode when format="percentage"), and metadata-driven configuration via objectCode/fieldCode props. Format property is read from XML metadata `<format>` tag. Labels automatically sourced from XML metadata or explicit `label` prop.
